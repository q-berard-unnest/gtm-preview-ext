  ...
  _resolveUserVariable(variable, context) {
    switch (variable.type) {
      ...
      case 'jsm': {
        ...
        // JSON.stringify encode fnBody comme un littéral de chaîne sûr,
        // évitant toute interaction syntaxique ({{Var}}, backticks, etc.).
        // new Function() exécute la définition de fonction dans le scope global,
        // comme GTM le fait nativement.
        const safeCode = `(function(){
          try {
            return (new Function('return (' + ${JSON.stringify(fnBody)} + ')'))()();
